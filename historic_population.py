...
def display(data, leaders, name_map, year):
    pop_totals = {}
    gdp_totals = {}
    next_pop_totals = {}
    next_gdp_totals = {}
            ...
            for period in periods:
                ...
                if year in range(start, end):
                    ...
                    try:
                        ...
                        next_gdp = info[year + 1]['gdppc'] * next_pop / 1e9
                    except KeyError:
                        next_pop = pop
                        next_gdp = gdp
    ...
    for ideology in pop_totals:
        pop_proportions[ideology] = round(pop_totals[ideology] / pop_total * 100, 1)
        gdp_proportions[ideology] = round(gdp_totals[ideology] / gdp_total * 100, 1)

    order = sorted(list(pop_totals.keys()), key=lambda n: pop_totals[n], reverse=True)
    size = 24
    print("-" * size * 8)
    print(str(year).rjust(size))
    print("Ideology".rjust(size),
          "Population (1000s)".rjust(size),
          "Population (%)".rjust(size),
          "GDP (trillions)".rjust(size),
          "GDP (%)".rjust(size),
          "GDP (per capita)".rjust(size),
          "GDP pc Growth (%)".rjust(size))
    for ideology in order:
        print(ideology.rjust(size),
              str(pop_totals[ideology]).rjust(size),
              str(pop_proportions[ideology]).rjust(size),
              str(round(gdp_totals[ideology], 1)).rjust(size),
              str(gdp_proportions[ideology]).rjust(size),
              str(int(gdp_totals[ideology] / pop_totals[ideology] * 1e9)).rjust(size),
              str(round(((next_gdp_totals[ideology] / next_pop_totals[ideology]) /
                         (gdp_totals[ideology] / pop_totals[ideology]) - 1) * 100, 1)).rjust(size))
    print("Total".rjust(size),
          str(pop_total).rjust(size),
          str("100.0".rjust(size)),
          str(round(gdp_total, 1)).rjust(size),
          str("100.0".rjust(size)),
          str(int(gdp_total / pop_total * 1e9)).rjust(size),
          str(round(((next_gdp_total / next_pop_total) / (gdp_total / pop_total) - 1) * 100, 1)).rjust(size))
